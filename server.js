...
console.log(OPENVIDU_SECRET, OPENVIDU_URL);
...
// Collection to pair session IDs with names
var mapSessionIDs = new Map();
        ...
        if (mapSessions[sessionName]) {
            ...
            OV.createSession(properties)
                .then(session => {
                    ...
                    // Store the new SessionID in the collection of Sessions
                    mapSessionIDs[session.sessionId] = session;
                    // Generate a new token asynchronously with the recently created tokenOptions
...
app.get('/api/sessions/:sessionId', function (req, res) {
    if (!isLogged(req.session)) {
        ...
        var relSession = mapSessionIDs[sessionId];
        relSession.fetch()
        .then(anyChange => {
            var activeConns = relSession.activeConnections;
            res.status(200).send(activeConns);
        })
    }
});
...
function login(user, pass) {
    return true; //(users.find(u => (u.user === user) && (u.pass === pass)));
}
